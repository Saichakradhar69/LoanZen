// src/app/existing-loan/loan-calculations/custom-loan.ts
...
// Note: This is very similar to the education loan calculation but provided
// as a separate function for future customization and clarity.
type LoanEvent = {
    date: Date;
...
export function calculateCustomLoan(data: ExistingLoanFormData): CalculationResult {
    const {
        disbursementDate,
        interestRate,
        moratoriumPeriod = 0,
    ...
    for (const event of events) {
        ...
        switch (event.type) {
            ...
            case 'rate-change':
                ...
                break;
            case 'repayment':
                if (isBefore(event.date, moratoriumEndDate)) continue;
    ...
    if (balance > 0 && data.paymentStructure === 'fixed') {
