...
function generateReportPdf(results: CalculationResults, userEmail: string): Buffer {
    ...
    const addFooter = () => {
        ...
        for(let i = 1; i <= pageCount; i++) {
            ...
            doc.text(`Report for ${userEmail} | Generated by LoanZen`, 14, pageHeight - 10);
        }
    };
    ...
    if (results.scenarios.length > 1) {
        ...
        doc.setFontSize(12);
        doc.setTextColor(0);
        doc.text('Key Finding:', 20, yPos + 8);
        doc.setFont('helvetica', 'normal');
        doc.text(`Choosing this option could save you`, 20, yPos + 15);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(0, 100, 0);
        doc.text(`$${savings.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`, 78, yPos + 15);
        doc.setTextColor(0);
        doc.setFont('helvetica', 'normal');
        doc.text(`in total interest.`, 128, yPos + 15);
        yPos += 38;
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(11);
        doc.text("This analysis compares two loan options based on the details you provided. The recommendation is based on minimizing the total cost of borrowing over the life of the loan.", 14, yPos, {maxWidth: 180});
        yPos += 20;

        doc.autoTable({
            startY: yPos,
            head: [['', ...results.scenarios.map(s => s.scenarioName), 'Difference']],
            body: [
                ['Monthly Payment', ...results.scenarios.map(s => `$${s.monthlyPayment.toFixed(2)}`), `$${(bestScenario.monthlyPayment - worstScenario.monthlyPayment).toFixed(2)}`],
                ['Total Interest', ...results.scenarios.map(s => `$${s.totalInterest.toFixed(2)}`), `$${(bestScenario.totalInterest - worstScenario.totalInterest).toFixed(2)}`],
                ['Total Cost of Loan', ...results.scenarios.map(s => `$${s.totalPayment.toFixed(2)}`), `$${(bestScenario.totalPayment - worstScenario.totalPayment).toFixed(2)}`]
            ],
        ...
        yPos = (doc as any).lastAutoTable.finalY;
         ...
         doc.text("A summary is provided when you compare two or more scenarios.", 14, yPos);
         yPos += 10;
    }
    ...
    const primaryScenario = results.scenarios[0];
    const schedule = primaryScenario.amortizationSchedule;

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.text(`Schedule for: ${primaryScenario.scenarioName}`, 14, yPos);
    yPos += 10;
    ...
    if (schedule.length <= 24) {
      body = schedule.map(row => [row.month, `$${row.principal.toFixed(2)}`, `$${row.interest.toFixed(2)}`, `$${row.remainingBalance.toFixed(2)}`]);
    } else {
      const first12 = schedule.slice(0, 12).map(row => [row.month, `$${row.principal.toFixed(2)}`, `$${row.interest.toFixed(2)}`, `$${row.remainingBalance.toFixed(2)}`]);
      const last12 = schedule.slice(-12).map(row => [row.month, `$${row.principal.toFixed(2)}`, `$${row.interest.toFixed(2)}`, `$${row.remainingBalance.toFixed(2)}`]);
      const separator = [['...', '...', '...', '...']];
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(11);
    doc.text("• Use this report to negotiate with your lender.", 20, yPos);
    yPos += 7;
    doc.text("• Consider the monthly payment impact on your budget.", 20, yPos);
    yPos += 7;
    doc.text("• Remember to account for any upfront fees not included in this analysis.", 20, yPos);
    yPos += 20;
    ...
    doc.setFillColor(245, 245, 245);
    doc.roundedRect(60, yPos, 90, 20, 3, 3, 'F');
